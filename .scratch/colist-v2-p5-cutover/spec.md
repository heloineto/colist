...
Status: done — migrated 2026-08-31, closed 2026-08-31 (HITL teardown checklist in the last comment)
...
3. All sessions reset — users log in again. Old Vercel URL (`colist.vercel.app`) → permanent 308 redirect to `https://colist.heloineto.com` (redirect-only `vercel.json`, Git integration disconnected).

### 5. Soak (1 week) + teardown
- Soak: watch CloudWatch logs + `errors` table; users on the new URL.
- After soak: Supabase project **paused** (delete +30 d), Vercel project **kept** as the 308 redirect (never deleted), `scripts/migrate-legacy.sh` + transform SQL deleted from the repo, Google consent screen **published** (leaves Testing).
...
- Soak complete; Supabase paused; Vercel project redirects; map updated.
...
- 2026-08-31 — Closed. Vercel decision changed: `colist.vercel.app` stays as a permanent 308 → `https://colist.heloineto.com` (redirect-only `vercel.json`, Git integration disconnected, no builds) — **project is never deleted**. Repo cleanup PR: `scripts/migrate-legacy*` gone, `.migration` un-ignored, dead cspell words, Supabase mention in `iac/modules/rds/variables.tf`; `password.util.ts` bcrypt-compat stays (migrated hashes depend on it). Soak evidence at close: see the recap in the PR. **HITL still pending (on/after 2026-09-07):** (1) Supabase `zgyllhgyslhshfbfujfu` → Pause; **delete on/after 2026-10-07**. (2) Google consent screen for `colist-web` → Publish. (3) Verify `colist.vercel.app/app` → 308 → new host; Google login with a non-test account. (4) Confirm only AWS bills: no Vercel builds, Supabase paused.
