  ...
  const lookup = async (email: string) => {
    ...
    try {
      ...
      modals.open({
        ...
        children: <MemberPreview list={list} user={user} />,
        onClose: () => form.reset(),
      });
    } catch (error) {
      ...
      notifications.show({
        color: 'red',
        message: notFound ? t('share.notFound') : t('errors.unexpected'),
      });
    }
  };
...
function MemberRow({
  ...
  list: ListsDtoOutputItem;
  member: MembersDtoOutputItem;
  canManage: boolean;
}) {
  const { t } = useTranslation();
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="min-w-0 grow">
        <p className="truncate text-sm font-bold">
          {member.name || t('share.noName')}
        </p>
        <p className="text-dimmed truncate text-sm">{member.email}</p>
      </div>
      <Badge
        variant="light"
        color={isOwner ? 'orange' : 'blue'}
        className="shrink-0"
      ...
      {canManage && (
        ...
        >
          <Menu.Target>
            ...
            >
              <DotsThreeVerticalIcon size="1.125rem" weight="bold" />
            </ActionIcon>
          </Menu.Target>
          <Menu.Dropdown>
            <Menu.Item
              ...
              onClick={() =>
                confirm({
                  ...
                  message: t('share.remove.message', {
                    member: member.name,
                    list: list.name,
                  }),
                  ...
                  onConfirm: () =>
                    remove.mutate({ listId: list.id, userId: member.userId }),
                })
              }
            >
              {t('share.remove.menu')}
  ...
  return (
    <>
      <ModalHeader
        icon={<UsersThreeIcon />}
        title={t('share.title')}
        description={t('share.subtitle', { name: list.name })}
      />
