...
export function MemberPreview({
  ...
  return (
    <div className="relative flex flex-col items-center gap-1 pt-8">
      <div
        className="absolute top-0 left-0 h-24 w-full rounded-t-md"
        style={{
          background: `var(--mantine-color-${getColor(getInitials(user.name))}-light-hover)`,
        }}
      />
      <UserAvatar
        name={user.name}
        image={user.image}
        size={100}
        className="border-body bg-body z-10 border-[5px] border-solid"
      />
      <p className="text-xl font-bold">{user.name || t('share.noName')}</p>
      <div className="xs:flex-row mt-4 flex w-full flex-col gap-2">
        <Button
          variant="default"
        ...
        <Button
          ...
          onClick={() =>
            add.mutate({ listId: list.id, data: { userId: user.id } })
          }
        >
          {t('share.addMember')}
        </Button>
