...
import { useListsDelete, useListsLeave } from '@/shared/api/generated/lists/lists';
...
export function useListActions(list: ListsDtoOutputItem | null) {
  ...
  const remove = useListsDelete({
    mutation: { meta: { success: t('lists.deleted') }, onSuccess: invalidateLists },
  });
  ...
  return {
    ...
    remove: () => confirmDelete(t('lists.deleteLabel'), () => remove.mutate({ listId: list.id })),
    leave: () =>
      confirm({
