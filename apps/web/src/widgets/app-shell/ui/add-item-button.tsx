...
export function AddItemNavButton({ expanded }: { expanded: boolean }) {
  ...
  return (
    ...
    >
      <span className="transition-opacity duration-150" style={{ opacity: expanded ? 1 : 0 }}>
        {t('shell.addItem')}
      </span>
    </Button>
  );
...
export function AddItemFooterButton() {
  ...
  return (
    <div className="relative -mt-8 flex items-start">
      <svg width="86" height="55" viewBox="0 0 86 55" className="absolute -top-0.5 left-1/2 -translate-x-1/2 fill-white dark:fill-dark-700" aria-hidden>
        <path d="M0 55C0 40 12 32 20 32c8 0 8-26 23-26s15 26 23 26c8 0 20 8 20 23z" />
      </svg>
      <ActionIcon variant="filled" radius="4.5rem" size="4.5rem" className="relative border-none!" onClick={add} aria-label={t('shell.addItem')}>
