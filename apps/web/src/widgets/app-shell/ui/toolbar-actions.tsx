...
export function FeedbackAction({ mode }: { mode: Mode }) {
  ...
  return (
    <Action
      ...
      onClick={() => feedback.open('feedback')}
    />
  );
}
  ...
  const links = (
    <>
      <NavLink
        component="button"
        className="px-4!"
        label={t('shell.editList')}
        disabled={!actions}
        leftSection={<NotePencilIcon size="1.125rem" />}
        onClick={run(() => actions?.edit())}
      />
      {actions?.isOwner !== false && (
        <NavLink
          component="button"
          className="px-4!"
          label={t('shell.deleteList')}
          disabled={!actions}
      ...
      {actions?.isOwner === false && (
        <NavLink
          component="button"
          className="px-4!"
          label={t('shell.leaveList')}
          leftSection={<SignOutIcon size="1.125rem" />}
          onClick={run(() => actions.leave())}
        />
      )}
      <NavLink
        component="button"
        className="px-4!"
        label={t('shell.reportError')}
        leftSection={<BugBeetleIcon size="1.125rem" />}
        onClick={run(() => feedback.open('error'))}
      />
    </>
  );
  ...
  return (
    <>
      ...
      <Drawer
        ...
        withCloseButton={false}
        classNames={{
          content: 'rounded-t-lg! h-fit!',
          body: 'px-0! py-2! flex flex-col',
        }}
      >
        {links}
      </Drawer>
